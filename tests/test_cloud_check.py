...
from tests.fakes_modal import FakeBillingSummary, FakeSDK
...
def test_a_healthy_target_authenticates_and_reads_its_workspace_and_credit(modal_toml):
    ...
    sdk.billing_summaries["ak-alice"] = FakeBillingSummary(
        metered_cost=7.5, billed_cost=0.0, start=cycle_start, end=cycle_end)
    ...
    assert check.credit_left == pytest.approx(22.5)
...
def test_a_positive_billed_cost_means_the_account_is_exhausted(modal_toml):
    modal_toml(alice="ak-alice")
    sdk = FakeSDK()
    sdk.billing_summaries["ak-alice"] = FakeBillingSummary(metered_cost=32.0, billed_cost=2.0)
...
def test_a_billing_summary_that_cannot_be_read_leaves_the_target_ok_with_no_credit_figures(modal_toml):
    ...
    assert check.credit_used is None and check.credit_left is None and check.exhausted is False
