...
- `GET /llms.txt` (and the identical `GET /agents.md`): the same agent guide as `pasar guide`, as plain text
...
- **Dashboard**: a header with the mascot and a one-line status ("2 jobs running, 2 waiting · next up #47 ~16:40"), stat tiles (memory pool, power, temperature, utilisation), then a **timeline** (memory on the vertical axis, time on the horizontal) of running jobs and the projected schedule of queued jobs; drag it or use its range buttons (6h, 1d, 1w, 30d) to look back at older jobs, and ctrl + scroll to zoom, then the **job table** (running, stopping, queued, recently finished).
