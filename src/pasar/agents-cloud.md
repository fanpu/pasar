...
- **`--on <group>` (e.g. `--on modal`) is the normal way to submit** when a target name covers
  several accounts: pasar picks which account pays, at submit; `--on <account>` pins a specific
  one instead. Either way the same rule above still applies: proposing and picking a target is
