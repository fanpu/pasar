...
class TargetCheck:
    ...
    workspace. `credit_used`/`credit_left`/`exhausted`/`cycle_start`/`cycle_end` are `None` (and
...
def _check_one(target: CloudTarget, sdk) -> TargetCheck:
    ...
    try:
        ...
        check.credit_used = float(s.metered_cost)
        check.exhausted = float(s.billed_cost) > 0
        check.cycle_start = s.start.timestamp() if s.start is not None else None
        check.cycle_end = s.end.timestamp() if s.end is not None else None
        check.credit_left = max(0.0, target.monthly_budget - check.credit_used)
    except Exception:
        log.exception("could not read %s's billing summary", target.name)
    return check
