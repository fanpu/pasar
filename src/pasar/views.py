...
def cloud_status_view(daemon, now: float, projection: dict) -> dict:
    ...
    *running* job whose own pace projects it past its approved run time (`needs_time`) — a
    ...
    `left`/`budget_exhausted` are the smaller of what the daily and the monthly budget still
    allow — the same two figures `cloud.lane.decide_cloud` gates a launch against — so `left` is
    what a job may still cost here right now and `budget_exhausted` is whether the budget gate
    would already block one. pasar's own ledger, not the provider's books.

    `credit_used`/`credit_exhausted`/`credit_as_of` are the provider's own books instead: what
    it says the account has used this cycle, whether it says the free allowance is gone, and
    when pasard last read that (`Daemon.cached_credit` — what pasard already holds, never a
    provider call from here). All three `None` when unknown: a provider that cannot say, a read
    not made yet or that failed, or a cycle that has since ended."""
    ...
    for name, target in sorted(daemon.cfg.clouds.items(), key=lambda kv: kv[1].group):
        ...
        credit = daemon.cached_credit(target)
        targets.append({
            "name": name,
            ...
            "running": running,
            "left": left,
            "budget_exhausted": left <= 0,
            "credit_used": credit[1].used if credit is not None else None,
            "credit_exhausted": credit[1].exhausted if credit is not None else None,
            "credit_as_of": credit[0] if credit is not None else None,
