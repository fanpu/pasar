...
| `pasar_job.progress(step, total_steps=None, **metrics)` | Report progress. Extra keyword args must be finite numbers (e.g. `loss=1.84`) — the UI charts each one as its own line on the job's Metrics tab. |
